...
/** @lends PixelPerfectActor */
{
  ...
  buildOverlay: makeInfallible(function(overlay) {
    ...
    overlay.imageContentNode = doc.insertAnonymousContent(container);
    overlay.imageContentNodeId = id;

    this.updateOverlay(overlay);
  }),

  updateOverlay: makeInfallible(function(overlay) {
    let content = overlay.imageContentNode;
    if (!content) {
      this.buildOverlay(overlay);
      return;
    }
    ...
    content.setAttributeForElement(overlay.imageContentNodeId,
      "style", styleStr);
  }),
